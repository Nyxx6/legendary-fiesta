...
if [ ! -d "$BASE_DIR/$CONTAINER_NAME/lower/bin" ]; then
    ...
    chmod 755 "$BASE_DIR/$CONTAINER_NAME/lower"
    debootstrap --variant=minbase --no-check-gpg stable "$BASE_DIR/$CONTAINER_NAME/lower" "$DEBIAN_MIRROR" || {
        echo "Erreur: debootstrap a échoué"
        echo "Essayez avec un répertoire différent, par exemple: /var/lib/containers"
        exit 1
    }
fi
...
unshare --mount --pid --net --uts --ipc --cgroup --fork --mount-proc /bin/bash -c "
    # Rendre les montages privés pour éviter la propagation
    mount --make-rprivate / 2>/dev/null || mount --make-private /
    
    # Bind mount merged sur lui-même pour en faire un vrai point de montage
    mount --bind '$BASE_DIR/$CONTAINER_NAME/merged' '$BASE_DIR/$CONTAINER_NAME/merged'
    
    # Changer vers le nouveau root
    cd '$BASE_DIR/$CONTAINER_NAME/merged'
    ...
    # Définir le hostname
    hostname '$CONTAINER_NAME' 2>/dev/null || export HOSTNAME='$CONTAINER_NAME'
