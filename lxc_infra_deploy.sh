...
# Exit on error and inherit ERR trap in functions/subshells
set -eE

# Generic error handler to make failures visible
handle_error() {
	local lineno=${1:-?}
	local cmd=${2:-?}
...
# Ensure required executables are available before starting
check_requirements() {
...
if [ $DELETE_ALL -eq 1 ]; then
	...
	# Remove containers (force) - ignore errors so cleanup continues
	...
	# Clean up generated config files and certificates
	rm -rf ssl_certs
...
server {
	...
	access_log /var/log/nginx/gil-access.log;
...
<body>
	...
	<script>
		function updateTime() {
			document.getElementById('time').textContent = 'Heure: ' + new Date().toLocaleString('fr-FR');
		}
		updateTime();
