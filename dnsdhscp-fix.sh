...
lxc exec client1 -- dig @192.168.20.10 router.lab.local +short

echo "[TEST] Client2 queries DNS Slave (local subnet)"
lxc exec client2 -- dig @192.168.20.10 router.lab.local +short

echo "[TEST] Zone transfer from Master to Slave"
lxc exec dns-master -- host -l lab.local 192.168.10.10
lxc exec dns-slave -- host -l lab.local 192.168.20.10
sleep 10
